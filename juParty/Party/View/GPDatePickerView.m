...
///Number of rows that are prevent by scroll picker to end
static NSInteger kBufforRows = 30;

@interface GPDatePickerView() <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation GPDatePickerView
...
- (NSDateFormatter *)dateFormatter {
    if (!_dateFormatter) {
        ...
        _dateFormatter.dateFormat = @"yyyy年 MMM dd日";
    }
...
- (NSInteger)minRowIndex {
    if (_minRowIndex == -1) {
        ...
        if ([self.minDate compare:[self dateForRow:kBufforRows]] == NSOrderedDescending) {
           ...
           NSLog(@"_minRowIndex:%ld",(long)_minRowIndex);
        }
    }
   NSLog(@"_minRowIndex:%ld",(long)_minRowIndex);
    return _minRowIndex;
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    switch (component) {
        ...
        case HourPicker:
            ...
            break;
        case MinutePicker:
            title = [NSString stringWithFormat:@"%02lu", row % [self realNumberOfRowsInComponent:component]];
