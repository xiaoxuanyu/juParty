...
//    NSLog(@"return: \n%@", result);
    ...
    if (!result) {
        NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                  error, @"error", nil];
        error = [NSError errorWithDomain:kDPAPIErrorDomain
                                    code:-1
                                userInfo:userInfo];
...
+ (NSString *)serializeURL:(NSString *)baseURL params:(NSDictionary *)params
{
	...
	if (CC_SHA1([stringBytes bytes], [stringBytes length], digest)) {
        ...
        NSLog(@"serializeURL:%@",[NSString stringWithFormat:@"%@://%@%@?%@", [parsedURL scheme], [parsedURL host], [parsedURL path], [paramsString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]);
