...
@property (strong, nonatomic) IBOutlet PBSaveBtn *rightButton;
...
+(void)show:(UIViewController *)handleVC type:(PhotoBroswerVCType)type index:(NSUInteger)index userID:(NSString *)userID photosWallArray:(NSMutableArray *)photosWallArray photoBroswerDelegate:(id<PhotoBroswerVCDelegate>)delegate photoModelBlock:(NSMutableArray *(^)())photoModelBlock{
    
    //取出相册数组
    NSMutableArray *photoModels = photoModelBlock();
    ...
    //记录
    pbVC.type =type;
    pbVC.photoBroswerDelegate=delegate;
    pbVC.photosWallArray=photosWallArray;
...
//    if ([self.photoBroswerDelegate respondsToSelector:@selector(rightButtonClick:)])  {
//        
//        [self.photoBroswerDelegate rightButtonClick:itemModel];
//    }
//    //读取缓存
...
//    }else{
        ...
        //展示提示框
        [CoreSVP showSVPWithType:CoreSVPTypeLoadingInterface Msg:@"删除中" duration:0 allowEdit:NO beginBlock:nil completeBlock:nil];
...
//           [CoreSVP showSVPWithType:CoreSVPTypeSuccess Msg:@"删除成功" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        [self deleteImageByUrl:itemModel];
    });
//    [itemView delete:^{
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            
//            [CoreSVP showSVPWithType:CoreSVPTypeSuccess Msg:@"删除成功" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
////            [self deleteImageByUrl:itemModel];
//            NSLog(@"itemModel.mid:%@",_photoModels);
//            [self dismiss];
//        
//           [self.photoModels removeObjectAtIndex:itemModel.mid-1];
//            if ([self.photoModels count]==1) {
//                self.index=0;
//            }else{
//                self.index=itemModel.mid-2;
//            }
////            if (itemModel.mid=1) {
////                self.index=1;
////            }else if(itemModel.mid!=1){
////            self.index=itemModel.mid-2;
////            }
//            NSLog(@"index:%d",self.index);
//            self.type=PhotoBroswerVCTypeZoom;
////            [self show];
//            
//            //取出相册数组
//            NSMutableArray *photoModels = self.photoModels;
//            
//            if(photoModels == nil || photoModels.count == 0) return;
//            
//            NSString *result= [PhotoModel check:photoModels type:self.type];
//            
//            if(result !=nil){
//                NSLog(@"result:%@",result);
//                return;
...
//            
//            
//            PhotoBroswerVC *pbVC = [[PhotoBroswerVC alloc] init];
//            NSLog(@"_photoModels.count%d",_photoModels.count);
//            if(self.index >= _photoModels.count){
//                NSLog(@"错误：index越界！");
//                return;
//            }
//            
//            //记录
//            pbVC.index = self.index;
//            
//            pbVC.photoModels = photoModels;
//            
//            //记录
//            pbVC.type =self.type;
//            pbVC.userID=self.userID;
//            pbVC.handleVC = self;
//            
//            //展示
//            [pbVC show];
...
////                
////                [self.delegate rightButtonClick:itemModel];
////            }
//        });
//    } failBlock:^{
//        [CoreSVP showSVPWithType:CoreSVPTypeError Msg:@"删除失败" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
//    }];
...
- (void)deleteImageByUrl:(PhotoModel *)itemModel{
  //http://192.168.20.138:8089/SSM/Front/gp_tupian_delete?picUrl=http://guangpaiwx.oss-cn-qingdao.aliyuncs.com/20150727-8d8f8405f1754.jpg
    ///请求参数
    NSMutableDictionary *params=[NSMutableDictionary dictionary];
    NSLog(@"itemModel.image_HD_U:%@",itemModel.image_HD_U);
    params[@"picUrl"]=itemModel.image_HD_U;
    [HttpTool postWithURL:Url_deleteImageByUrl params:params success:^(id json) {
        NSLog(@"responseObject:%@",json);
        NSDictionary *dic=json;
    if (dic[@"success"]) {
//        [_photoModels removeObjectAtIndex:itemModel.mid];
        [CoreSVP showSVPWithType:CoreSVPTypeSuccess Msg:@"删除成功" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
        //            [self deleteImageByUrl:itemModel];
        NSLog(@"itemModel.mid:%@",_photoModels);
        [self dismiss];
        ...
        [self.photoModels removeObjectAtIndex:itemModel.mid-1];
        [self.photosWallArray removeObjectAtIndex:itemModel.mid-1];
        if ([self.photoModels count]==1) {
            self.index=0;
        }else{
            self.index=itemModel.mid-2;
        ...
        pbVC.type =self.type;
        pbVC.userID=self.userID;
        pbVC.photoBroswerDelegate=self.photoBroswerDelegate;
        pbVC.photosWallArray=self.photosWallArray;
        pbVC.handleVC = self;
...
- (void)setPhotosWallArray:(NSMutableArray *)photosWallArray{
     _photosWallArray = photosWallArray;
}
/** 取出可重用照片视图 */
...
-(void)dismiss{
    ...
    if ([self.photoBroswerDelegate respondsToSelector:@selector(leftButtonClick:)])  {
        
        [self.photoBroswerDelegate leftButtonClick:_photosWallArray];
    }
